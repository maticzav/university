...
% Kogar zanima veÄ LaTeXu, lahko obiÅ¡Äe https://www.latex-project.org

% LaTeX sprejme eno ali veÄ datotek .tex in naredi PDF.
% V datoteko .tex piÅ¡emo besedilo in ukaze.

% Komentarji se zaÄnejo z znakom % in veljajo do konca vrstice.

% Dokument se zaÄne z ukazom \documentclass.
...
%   article - Älanek
%   amsart - Älanki za zdruÅ¾enje American Mathematical Society
%   beamer - prosojnice za predavanje
%   book - knjiga
%   letter - pismo

% Nastavitve so odvisne od tipa dokumenta, nekatere obiÄajne so:
% a4paper ... format strani naj bo A4 (namesto ameriÅ¡ki letter)
% 10pt, 11pt, 12pt ... velikost pisave

% Sledi PREAMBULA, kjer se Å¡e ne piÅ¡e nobenega besedila, ampak
% samo ukaze in nastavitve.

% LaTeX ima veliko Å¡tevilo zelo uporabnih paketov. ObiÄajno najprej
% povemo, katere pakete bomo uporabljali. To naredimo z ukazom
% \usepackage[...]{...} kjer v [...] damo dodatne nastavitve za paket
% in v {...} ime paketa.

% Za slovenÅ¡Äino moramo nastaviti kodno tabelo datoteke .tex in
% kodno tabelo datoteke PDF, da se pravilno obravnava Å¡umnike.
% LaTeXu povemo tudi, da uporabljamo slovenÅ¡Äino (da pravilno prikaÅ¾e datum ipd).

\usepackage[utf8]{inputenc}   % vhodna datoteka je kodirana v sistemu UTF-8

% Ostale moÅ¾nosti za kodiranje so:
...
\usepackage[slovene]{babel}   % uporabljamo slovenÅ¡Äino

% Nastavimo lahko tudi razne pisave, glej http://www.tug.dk/FontCatalogue/
% Za slovenÅ¡Äino je dobro nastaviti novejÅ¡o inaÄico pisave lmodern

\usepackage{lmodern}            % BoljÅ¡i fonti, s pravimi Å¡umniki
...
% S tem smo zakljuÄili pakete in nastavitve za slovenÅ¡Äino. Ostali paketi
% in nastavitve so odvisni od tega, kaj bomo poÄeli. V tej datoteki bomo
...
% Sledi vsbina dokumenta, ki se zaÄne z ukazom \begin{document} in konÄa z \end{document}.
...
% Lahko bi nastavili tudi datum, Äe ga ne, se bo izpisal danaÅ¡nji datum
% \date{10.\ 10.\ 2017}

% Ukaz, s katerim dejansko naredimo naslov
\maketitle

% LaTeX pozna t.i. okolja, ki se zaÄnejo in konÄajo z ukazoma
...
% ÄŒlanek je hierarhiÄno razdeljen na razdelke (\section), podrazdelke (\subsection),
% podpodrazdelke (\subsubsection) in podpodpodrazdelke (\paragraph). V veÄini primerov
% potrebujemo samo razdelke in podarazdelke.

\section{Uporaba razdelkov}

% Oblikovalci pogosto potrebujejo kako besedilo, da prikaÅ¾ejo design spletne strani
...
% Ukazi s katerimi nastavimo velikost pisave, a tega raje ne poÄnite na roke
\begin{center}
{\Huge Zdravljica} \\
...
\texttt{\char126}. Ta presledek prepreÄi prelom vrstice in ga uporabimo, kadar bi bil
...
stavka. To naredimo tako, da pred presledek zapiÅ¡emo poÅ¡evnico \texttt{\char92} ali tildo
\texttt{\char126}. Tako piÅ¡emo N.~Tesla in ne N. Tesla. VÄasih si ne moremo privoÅ¡Äiti
...
\item \emph{prikazni naÄin} (display math), kjer je izraz zapisan med \texttt{{\char96}[} in
