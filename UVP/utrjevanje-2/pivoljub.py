...
def slovar_cen(seznam):
    return {pivo: cena for (pivo, _, _, cena, _) in seznam}
...
def izvor_in_znamka(seznam):
    analiza = defaultdict(dict)
    for (pivo, alk, drzava, cena, stil) in seznam:
        analiza[drzava][pivo] = (cena, alk)
    return dict(analiza)
...
# =============================================================================
def zapisi_podatke(izhodna, slovar):
    # pretvori v seznam
    seznam = []
    for drzava in slovar:
        for pivo in slovar[drzava]:
            cena, alk = slovar[drzava][pivo]
            seznam.append((pivo, alk, drzava, cena))
    # razvrsti
    seznam.sort(key=lambda pivo: (-pivo[1], pivo[0]))
    # izpisi
    with open(izhodna, "w", encoding="utf-8") as f:
        for (pivo, alk, drzava, cena) in seznam:
            print("{} ({}) {}".format(pivo, drzava, alk), file=f)
...
# =============================================================================
def zapitek(prijatelji, cene):
    zapito = {}
    for prijatelj in prijatelji:
        zapito[prijatelj] = 0.0
        for pivo in prijatelji[prijatelj]:
            if pivo in cene:
                zapito[prijatelj] += cene[pivo]
            else:
                zapito[prijatelj] = None
                break
    return dict(zapito)
...
zapitek(
    {
        "Jure": ["Bernard Bohemian Ale 16", "Blanche De Bruxelles"],
        "Nino": ["Carniola ESB", "Carniola ESB", "Carniola ESB", "Carniola ESB"],
        "Ines": ["Blanche De Bruxelles", "Laško"],
        "Anja": [],
    },
    {
        "Bernard Bohemian Ale 16": 2.2,
        "Blanche De Bruxelles": 1.72,
        "Carniola ESB": 2.6,
        "Brewdog Candy Kaiser": 2.16,
    },
)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJ1c2VyIjo1MzU2LCJwYXJ0IjoyMTAxMn0:1jMrXI:JzzYxjRoGjFwPx8kRrSY62GruJY"
        try:
            Check.equal(
                """slovar_cen([('Brewdog Candy Kaiser', 5.2, 'Škotska', 2.16, 'Altbier'),
                ('Carniola ESB', 5.2, 'Slovenija', 2.60, 'ESB'),
                ('Blanche De Bruxelles', 4.5, 'Belgija', 1.72, 'Belgian White'),
                ('Bernard Bohemian Ale 16', 8.2, 'Češka', 2.20, 'Belgian Strong Ale')])""",
                {
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJ1c2VyIjo1MzU2LCJwYXJ0IjoyMTAxM30:1jMrXI:W4z0owfjS_qlhkN5osggmtInkso"
        try:
            Check.equal(
                """izvor_in_znamka([('1910 Double Stout', 7.4, 'Anglija', 3.50, 'Foreign Stout'),
                ('Viven Imperial IPA', 8.0, 'Belgija', 3.09, 'Imperial IPA'),
                ('Blanche De Bruxelles', 4.5, 'Belgija', 1.72, 'Belgian White'),
                ('Banana Bread Beer', 5.2, 'Anglija', 2.27, 'English Strong Ale')])""",
                {
                    "Belgija": {
                        "Viven Imperial IPA": (3.09, 8.0),
                        "Blanche De Bruxelles": (1.72, 4.5),
                    },
                    "Anglija": {
                        "1910 Double Stout": (3.5, 7.4),
                        "Banana Bread Beer": (2.27, 5.2),
                    },
                },
            )
        except:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJ1c2VyIjo1MzU2LCJwYXJ0IjoyMTAxNH0:1jMrXI:jXnjVal5pv8Sa4TfUXKUBaFgQDY"
        try:
            test_cases = [
                (
                    {
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJ1c2VyIjo1MzU2LCJwYXJ0IjoyMTAxNX0:1jMrXI:qQzo77N6IXdZGBUs2a46O4Zgz2I"
        try:
            Check.equal(
                """zapitek({'Jure': ['Bernard Bohemian Ale 16', 'Blanche De Bruxelles'],
                'Nino': ['Carniola ESB', 'Carniola ESB', 'Carniola ESB', 'Carniola ESB'],
                'Ines': ['Blanche De Bruxelles', 'Laško'], 'Anja': []},
                {'Bernard Bohemian Ale 16': 2.2, 'Blanche De Bruxelles': 1.72,
                'Carniola ESB': 2.6, 'Brewdog Candy Kaiser': 2.16})""",
