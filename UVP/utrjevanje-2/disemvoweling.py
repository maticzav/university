...
#     4
# =============================================================================
def prvi_samoglasnik(niz: str):
    for i, znak in enumerate(niz):
        if znak.upper() in "AEIOU":
            return i
    return -1
...
def disemvowel(niz):
    samoglasniki = ""
    # naberi samoglasnike na kup
    while prvi_samoglasnik(niz) >= 0:
        i = prvi_samoglasnik(niz)
        samoglasniki += niz[i]
        niz = niz[:i] + niz[i + 1:]
    return niz + samoglasniki

# KOMENTAR: tale rešitev ni najbolša ker prvi_samoglasnik vedno išče od
#           začetka besedila
...
def razveljavi_disemvowel(niz):
    # preberi samoglasnike, vrni niz, ce ni samoglasnikov
    i = prvi_samoglasnik(niz)
    if i == -1:
        return niz
    samoglasniki = niz[i:]
    ...
    for znak in niz:
        if znak == "*":
            desifriran_niz += samoglasniki[0]
            samoglasniki = samoglasniki[1:]
        else:
            desifriran_niz += znak
    return desifriran_niz
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoyNDIyNX0:1jMq5k:GV37e6js5wZ9akC6BMho4RIn6_o'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoyNDIyNn0:1jMq5k:qPHXATDwfkEGAU1i-TeufUGUwRg'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoyNDIyN30:1jMq5k:uouCaNxB-xLNRrtbyHB7-IVxdBM'
        try:
