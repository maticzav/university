...
def vsota_produktov_sosednjih(n):
    return sum([k * (k + 1) for k in range(n + 1)])
...
def stevilo_clenov(m):
    if m < 1:
        return 0
    st_clenov = 1
    while vsota_produktov_sosednjih(st_clenov) <= m:
        st_clenov += 1
    return st_clenov - 1
...
def razdalja(k, m):
    ...
    while vsota < m:
        vsota += (k + i) * (k + i + 1)
        i += 1
    # vrnemo najmanjso razdaljo okliških vrednosti
    return min(vsota - m, m - vsota + (k + i) * (k + i - 1))

def najblizje(a, b, m):
    # primerjamo vse glede na razdaljo
    return min(range(a, b + 1), key=lambda k: razdalja(k, m))
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoxNDc0fQ:1jMq5n:PC7CgiHZTOdi6uHRoenjzngl8vQ'
        try:
            test_data = [
                ("""vsota_kvadratov(1)""", 1),
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoxNDc1fQ:1jMq5n:OCVdosN-jqEFlVIpQnm3eq2lG-s'
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoxNDc2fQ:1jMq5n:DfcTWIdKuiMaVI7bZmBvKdcGBS4'
        try:
            test_data = [
                ("""stevilo_clenov(2)""", 1),
                ("""stevilo_clenov(19)""", 2),
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoxNDc3fQ:1jMq5n:xMmB4ISur90KRTjoYSPFQvTztVg'
        try:
            test_data = [
