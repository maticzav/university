...
def cik_cak(niz: str, st_vrstic=3):
    niz = [znak for znak in niz.upper() if znak.isalpha()]
    # vstavi prazne vrstice
    vrstice = [""] * st_vrstic
    # sprocesiraj znake
    for i, znak in enumerate(niz):
        # modeliraj sinusno krivuljo
        n = st_vrstic - 1
        vrstica = round(abs((n * 2) / math.pi * math.asin(math.sin(i * math.pi / (2 * n)))))
        # pripisi znake v vrstice
...
# KOMTENAR: Tole je mal overkill za tole nalogo haha.
# besedilo = "Danes je lep sončen dan. Medvedova družina gre na sprehod."
# for n in range(4, 10):
#     print(n, '-' * 30)
#     print()
#     for vrstica in list(cik_cak(besedilo, n)):
#         print(vrstica)
#     print()
...
#     'ACDTAKTANTAW'
# =============================================================================
def cik_cak_sifra(niz: str, st_vrstic=3):
    cikcak = cik_cak(niz, st_vrstic)
    return "".join(list(cikcak)).replace(".", "")
...
# =============================================================================
def razrez(niz: str):
    return [podniz.strip() for podniz in niz.split()]
...
#     temno, žita ne bo.
# =============================================================================
def olepsanoBesedilo(niz: str, ll=90):
    kurzor = 0
    besedilo = [""]
    for beseda in razrez(niz):
        # dodaj k zadnji vrstici
        if kurzor + len(beseda) <= ll:
            besedilo[len(besedilo) - 1] += "{} ".format(beseda)
            kurzor += len(beseda) + 1
        else:
            besedilo.append("{} ".format(beseda))
            kurzor = len(beseda) + 1
    return "\n".join([vrstica.strip() for vrstica in besedilo])
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoxNDkyfQ:1jMq5v:KmcZKVeM2fyxv2zTJtPLxXENeGE'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoxNDkzfQ:1jMq5v:iCpCFDT2d47ZoGJwct2jjqZGjjw'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoxNDk0fQ:1jMq5v:OjWHyPGIYoCjeyEd3SFFGzmF-i8'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoxNDk1fQ:1jMq5v:Lr_dkQienG0XoOoKx853JDA_Dzs'
        try:
