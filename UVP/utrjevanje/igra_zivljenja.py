...
# =============================================================================
def zivi(svet, i, j):
    # zacnemo z najmanj 0 da ne gremo od zadaj
    vrstice = svet[max(0, i - 1):i + 2]
    stolpci = [sum(vrstica[max(0, j - 1):j + 2]) for vrstica in vrstice]
    # odstejemo sebe
    return sum(stolpci) - svet[i][j]
...
def igra(svet):
    ...
    for i in range(len(svet)):
        ...
        for j in range(len(vrstica)):
            # smo v dani celici (i, j)
            sosedje = zivi(svet, i, j)
            ziva = svet[i][j]
            if ziva and (sosedje < 2 or sosedje > 3):
                nova_vrstica.append(False)
            elif not ziva and sosedje == 3:
                nova_vrstica.append(True)    
            else:
                nova_vrstica.append(ziva)
        nov_svet.append(nova_vrstica)
    return nov_svet
...
# celic v matriki.
# =============================================================================
def pop_generacije(svet):
    return sum([sum(vrstica) for vrstica in svet])

def populacija(svet, n):
    generacije = []
    while n > -1:
        generacije.append(pop_generacije(svet))
        svet = igra(svet)
        n -= 1
    return generacije
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0Ijo5OTY5fQ:1jJMGQ:fPrYU4QExcIT5NHSfPY4Pr7ayUw'
        try:
            svet_1 = [
                [False, False, False, False, False, False],
                [False, False, False, True, False, False],
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0Ijo5OTcwfQ:1jJMGQ:5dLi3jBHtS5Nr5U9N8k8GuXvrK4'
        try:
            svet_1 = [
                [False, False, False, False, False, False],
                [False, False, False, True, False, False],
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0Ijo5OTcxfQ:1jJMGQ:6Zg9CkVYctUfBxQswtWc8xJjxOM'
        try:
            svet_1 = [
                [False, False, False, False, False, False],
                [False, False, False, True, False, False],
