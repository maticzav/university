...
#     4
# =============================================================================
def samoglasniki(niz: str):
    return len([crka for crka in niz if crka.upper() in "AEIOU"])
...
def haiku(niz):
    ...
    niz = niz.split("\n")
    for n in range(3):
        if samoglasniki(niz[n]) != oblika[n]:
            return False
    return len(niz) == 3
...
def sonet(niz):
    niz = niz.split("\n")
    st_vrstic = (4 + 1) + (4 + 1) + (3 + 1) + 3
    if len(niz) != st_vrstic:
        return False
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoyMTAwMX0:1jJMGU:P1XvT0-ovv8gVdVN-9cwKIQHYls'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoyMTAwMn0:1jJMGU:ys8JsrRrbmVg-gByNkJUmI2ji7k'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoyMTAwM30:1jJMGU:gglclyZj8fM6igxWfQyElz_5sJA'
        try:
            test_data = [
                ('vrstice("Danes\\n je lep\\ndan.\\n")', ["Danes", " je lep", "dan.", ""]),
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoyMTAwNH0:1jJMGU:3T0CFNAK2H3NkRMYXFghoCMpHlI'
        try:
            ...
            Check.equal("""haiku('aa\\naa')""", False)
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])

    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoyMTAwNX0:1jJMGU:pC-Q0z1x_BvwgPxWoSALTOFyM4I'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoyMTAwNn0:1jJMGU:1lZX-6d1_faoR7RhAWJfZkUdZaA'
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoyMTAwN30:1jJMGU:Q4oAQydOXoEO3pAl9L9OF_BHbUo'
        try:
            test_data = [
