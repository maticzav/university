...
def znesek_racuna(artikli, racun):
    ...
    for artikel in racun:
        (kolicina, popust) = racun[artikel]
        (cena_artikla, ddv) = artikli[artikel]
        cena += (cena_artikla * kolicina) * (1 - (popust / 100))
    return round(cena, 2)
...
def davcni_obracun(artikli, racun):
    ...
    for artikel in racun:
        (kolicina, popust) = racun[artikel]
        (cena_artikla, ddv_stopnja) = artikli[artikel]
        cena = (cena_artikla * kolicina) * (1 - (popust / 100))
        davek = davcna_osnova(cena, ddv_stopnja)

        obracun[ddv_stopnja] = obracun.get(ddv_stopnja, 0) + davek
    # zaokroži
    for stopnja in obracun:
        obracun[stopnja] = round(obracun[stopnja], 2)
    return obracun
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0Ijo5OTY1fQ:1jJMGZ:aY3kgucl_92Htt6BocDNGqbW0vQ'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0Ijo5OTY2fQ:1jJMGZ:j5D99xmmcIOfOoY5sWiko93lz-0'
        try:
            artikli = {
                'ponev': (32.74, 22),
                'knjiga': (18.60, 9.5),
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0Ijo5OTY3fQ:1jJMGZ:ETv3o8uo_gTaUvkJIDoA4IXVr6I'
        try:
            artikli = {
                'ponev': (32.74, 22),
                'knjiga': (18.60, 9.5),
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0Ijo5OTY4fQ:1jJMGZ:u6qOkzRQb6HwIgBMgR1cRovuRo8'
        try:
            artikli = {
                'ponev': (32.74, 22),
                'knjiga': (18.60, 9.5),
