...
#     False
# =============================================================================
def je_seznam_ciklov(xxs):
    perm = sum(xxs, [])
    vsi_pozitivni = all([x > 0 for x in perm])
    notri = []
    for x in perm:
        if x in notri:
            return False
...
def urejeni_cikli(cikli):
    ...
    for cikel in cikli:
        if len(cikel) > 1:
            i = cikel.index(min(cikel))
            sez.append(cikel[i:] + cikel[:i])
    sez.sort()
    return sez
...
def iz_ciklov(cikli, dolzina=0):
    ...
    # Permutacija
    for cikel in cikli:
        for i, clen in enumerate(cikel):
            if (i + 1) == len(cikel):
                perm[clen - 1] = cikel[0]
            else:
                perm[clen - 1] = cikel[i + 1]
    return perm
...
def red(cikli):
    v = 1
    for cikel in cikli:
        v = (v * len(cikel)) // math.gcd(v, len(cikel))
    return v
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoxMzQwNH0:1jDESf:JSAdV5pz2FzOLoKOurmQTafM_cw'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoxMzQwNX0:1jDESf:l95JN3M0a6jBPLoFyLN69z1e6Fc'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoxMzQwNn0:1jDESf:0zm8wvPo7EobnhgmP4rUUCJpU30'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoxMzQwN30:1jDESf:AUpVFBm-9PJbYtShSle1dI5PdNo'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoxMzQwOH0:1jDESf:_Jh4uoz2jaxYmojWsEQUCC5Q6xM'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoxMzQwOX0:1jDESf:iIfhicVwcBYn5es-JS_vj8aZG0I'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoxMzQxMH0:1jDESf:Jy8XOGzC256QQWkRWTLYAZKRUjU'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoxMzQxMX0:1jDESf:0rzhiYIQMC7TaVVzaMMhyWqr0rQ'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoxMzQxMn0:1jDESf:DgXGHYJvWludkrSA-JYBUEGk2l4'
        try:
