...
def narcisoidi(omrezje):
    return [oseba for oseba, vsecki in omrezje.items() if oseba in vsecki]
...
def pari(mreza):
    pari = []
    for oseba, ljubljeni in mreza.items():
        for ljubljen in ljubljeni:
            if oseba in mreza.get(ljubljen, []):
                # zaljubljenca
                if oseba <= ljubljen:
                    pari.append((oseba, ljubljen))
    return pari
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoxMzg0fQ:1jG4e0:hISq-YeeLrRyM8idHgcwqKf-Zt8'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoxMzg1fQ:1jG4e0:OOgtEAHn7azuTRV-UjHi2SLoUdY'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoxMzg2fQ:1jG4e0:4K1w-R83k1DkY1rvJUKgXNF3w8Q'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoxMzg3fQ:1jG4e0:945UCaNVRsYySrklIW0SPo0yqaY'
        try:
