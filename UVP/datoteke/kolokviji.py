...
def nalozi_csv(datoteka):
    ...
    with open(datoteka) as d:
        for vrstica in d:
            rezultati.append(nabor(vrstica.strip()))
    return rezultati
...
def vsote(vhodna, izhodna):
    ...
    with open(izhodna, "w") as f:
        for tocke in csv:
            print("{},{}".format(tocke[0], sum(tocke[1:])), file=f)
...
# =============================================================================
def povprecje_naloge(tocke, n):
    return "{:.2f}".format(sum([rezultat[n] for rezultat in tocke]) / len(tocke))

def rezultati(vhodna, izhodna):
    csv = nalozi_csv(vhodna)
    # uredi po priimkih
    csv.sort(key=lambda rezultat: rezultat[0].split()[1])
    csv = [tuple(list(tocke) + [sum(tocke[1:])]) for tocke in csv]
    
    # dodaj povprecje
    statistika = ["POVPRECEN STUDENT"]
    for i in range(1, len(csv[0])):
        statistika.append(povprecje_naloge(csv, i))
    csv.append(tuple(statistika))
    # dodaj skupne tocke
    with open(izhodna, "w") as f:
        # izpisi rezultate
        for tocke in csv:
            print("{},{},{},{},{},{},{}".format(*tocke), file=f)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoxNTE4fQ:1jLLAi:yT8HgSLCM65--tpPe1AmQHVEG18'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoxNTE5fQ:1jLLAi:3k341oB9lkNKMbDXQacoMZXQBtc'
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoxNTIwfQ:1jLLAi:bJ0dEN489MIoNpsz4CqedEpyJA8'
        try:
            test_data = [
                ("kolokviji_vhod.txt", ["Janez Novak,1,3,3,2,0", "Micka Kovaceva,0,3,2,3", "Miha Praznic", "Peter Klepec,1,0,1,2,1,3"],
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoxNTIxfQ:1jLLAi:HL15Lx3DhIN-dUEMHy6GjoiGYAM'
        try:
            test_data = [
