...
#     2
# =============================================================================
def kolikokrat_se_pojavi(imena, ime):
    return imena.split(",").count(ime)
...
def koliko(niz, datoteka):
    ...
    with open(datoteka, "w") as d:
        for ime in niz.split(","):
            if ime not in presteti and ime != "":
                print(ime, kolikokrat_se_pojavi(niz, ime), file=d)
                presteti.append(ime)
...
def koliko_urejen(vhodna, izhodna):
    ...
    obiskani = []
    for ime in imena:
        if ime not in obiskani and ime != '':
            obiskani.append(ime)
            pojavitve.append((ime, imena.count(ime)))
    # izpisi urejene
    pojavitve.sort(key=lambda par: (-par[1], par[0]))
    with open(izhodna, "w") as d:
        for ime, st in pojavitve:
            print(ime, st, file=d)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoxNTEwfQ:1jLLAe:z_d_Ge0l7a03jDGG4D3cZXCEd6Y'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoxNTExfQ:1jLLAe:h8vPvv_DKqwS7FOI_piv2qQciR0'
        try:
            test_cases = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoxNTEyfQ:1jLLAe:1FI6homRh8ZInhTjYwmgaEF61jw'
        try:
            test_cases = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoxNTEzfQ:1jLLAe:ns-iaOMxaXbm94q4GUf-csA03hI'
        try:
            test_cases = [
