...
def najpogostejse_besede(vhod, st_besed, izhod):
    with open(vhod) as d:
        besede = d.read().split()
    pojavitve = []
    for beseda in set(besede):
        pojavitve.append((beseda, besede.count(beseda)))
    pojavitve.sort(key=lambda par: (-par[1], par[0]))
    with open(izhod, "w") as f:
        for i in range(st_besed):
            beseda, st = pojavitve[i]
            print(beseda, st, file=f)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0Ijo5ODkxfQ:1jLLAo:xwKJ8WcSwEiU7D7evW9qxL1Gq08'
        try:
            pass
        except:
