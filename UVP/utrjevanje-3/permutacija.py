...
class Permutacija:
    def __init__(self, cikli, stopnja=None):
        # nastavi stopnja na najvecji element ce ni podana
        self.stopnja = stopnja or max(max(*cikli))
        # uredi cikle
        sez = []
        for cikel in cikli:
            if len(cikel) > 1:
...
class Permutacija(Permutacija):
    def inverz(self):
        ...
        for cikel in self.cikli:
            inv.append(cikel[::-1])
        return Permutacija(inv, stopnja=self.stopnja)
...
def lcm(ns):
    ...
    for i in range(len(ns)):
        lcm = ns[i] * lcm // math.gcd(ns[i], lcm)
    return lcm


class Permutacija(Permutacija):
    def red(self):
        return lcm([len(cikel) for cikel in self.cikli])
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoyMTQ1fQ:1jT17i:7dEPT5OubCFMBp67-O_asGdLWts'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoyMTQ2fQ:1jT17i:UkgvOzX5DwkuaZE1XLDEP7rR_z8'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoyMTQ3fQ:1jT17i:uE1SO5AG8UVj-yy-mMjpW-9ntdo'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoyMTQ4fQ:1jT17i:SvxZc0StivL13NLif_qB9YvSqZI'
        try:
