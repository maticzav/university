...
class Mnogokotnik(Mnogokotnik):
    def obseg(self):
        o = 0
        # zacnes na predzadnjem da pridemo pravilno okrog
        for n in range(0, len(self.oglisca)):
            (px, py) = self.oglisca[n - 1]
            (x, y) = self.oglisca[n]
            # razdalja med prejsnjim in tem
            r = ((x - px) ** 2 + (y - py) ** 2) ** (1 / 2)
            o += r
        return o
...
class Mnogokotnik(Mnogokotnik):
    def ploscina(self):
        d = 0
        for i in range(len(self.oglisca)):
            (x1, y1) = self.oglisca[i - 1]
            (x2, y2) = self.oglisca[i]
            d += x1 * y2 - x2 * y1
        return d / 2
...
class Mnogokotnik(Mnogokotnik):
    def je_konveksen(self):
        ...
        for i in range(len(self.oglisca)):
            # pridobi ogliscaj
            (px, py) = self.oglisca[i - 2]
            (qx, qy) = self.oglisca[i - 1]
            (rx, ry) = self.oglisca[i]
            # izračunaj vektorja
            (v1, v2) = (px - qx, py - qy)
            (w1, w2) = (qx - rx, qy - ry)
            # izracunaj smer z vektorkim produktom
            z = v1 * w2 - v2 * w1
            # pristej smeri
            s += z / abs(z)
        return abs(s) == len(self.oglisca)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoyMTM1fQ:1jTOkk:UFBY5YMDA5KtzFZcyozrt_1Ww18'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoyMTM2fQ:1jTOkk:4XRTNOdCwX3TJhBthKuGTcId5WM'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoyMTM3fQ:1jTOkk:f0ZWmKbHEBq7uM5wg5gKeirF1fs'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzU2LCJwYXJ0IjoyMTM4fQ:1jTOkk:pj5doM_QghGsDiYH8Q2whfR7y9E'
        try:
